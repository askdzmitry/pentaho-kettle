...
/**
 * This class is used to read repository, load jobs and transformations, and export them into xml file. Xml file will be
 * overwrite. In case of not success export this file will be deleted.
 * 
 ...
 * 
 * In case of during export some item form repository will be failed to load in case of missing step plugin etc - same
 * as before - whole export process will be treated as failed, export xml file will be deleted.
 * 
...
public class RepositoryExporter implements IRepositoryExporterFeedback {
    ...
    try {
      ...
      for ( int d = 0; d < dirids.length; d++ ) {
        ...
        log.logDebug( BaseMessages.getString( PKG,
            "Repository.Exporter.Log.FindJobs", jobs.length, repdir.getName() ) );
        ...
        for ( int i = 0; i < jobs.length; i++ ) {
          ...
          List<ImportValidationFeedback> errors = this.validateObject( jobMeta );
          if ( errors.isEmpty() ) {
  ...
  private List<ImportValidationFeedback> validateObject( Object subject ) {
    if ( !hasRules ) {
  ...
  private void exportTransformations( ProgressMonitorDecorator monitor, RepositoryDirectoryInterface dirTree,
    ...
    try {
      ...
      for ( int d = 0; d < dirids.length; d++ ) {
        ...
        log.logDebug( BaseMessages.getString( PKG,
            "Repository.Exporter.Log.FindTrans", trans.length, repdir.getName() ) );
        ...
        for ( int i = 0; i < trans.length; i++ ) {
          ...
          List<ImportValidationFeedback> errors = this.validateObject( transMeta );
          if ( errors.isEmpty() ) {
  ...
  enum XmlElements {
    REPO_START( "<repository>" + Const.CR + Const.CR ), REPO_END( "</repository>" + Const.CR + Const.CR ), JOBS_START(
        "<jobs>" + Const.CR ),
        JOBS_END( "</jobs>" + Const.CR ), TRANS_START( "<transformations>" + Const.CR ), TRANS_END(
        "</transformations>" + Const.CR );
